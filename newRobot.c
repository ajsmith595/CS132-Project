...
void setServoAngle(unsigned char id, float deg){
	mtl(id, radsToRobotArmServoValue(deg));
}
...
void setVertical(int block){
	...
	for(i = 0; i < 3; i++){
		...
		float angle = PI / 2 - getPointAngle(diff);
		printf("Angle: %.6f\n", radsToDegrees(angle - prevAngle));
		//setServoAngle(i + 2, angle - prevAngle);
		prevAngle = angle;
	}
}
